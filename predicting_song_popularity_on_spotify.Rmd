---
title: 'Exploratory Analysis and Model Building'
output:
...
# EDA, Missing Data & Multivariate Imputation by Chained Equations
...
geom_miss_point() plots are a useful approach to further explore the three missing data mechanisms (MCAR, MAR, NMAR) and distribution of the observations for each audio feature.
...
```{r missingness of Duration ms by Song Popularity, echo=FALSE}
...
```{r missingness of Acousticness by Song Popularity, echo=FALSE}
...
```{r missingness of Danceability by Song Popularity, echo=FALSE}
...
```{r missingness of Energy by Song Popularity, echo=FALSE}
...
```{r missingness of Instrumentalness by Song Popularity, echo=FALSE}
...
```{r missingness of Liveness by Song Popularity, echo=FALSE}
...
```{r missingness of Loudness  by Song Popularity, echo=FALSE}
...
```{r missingness of Speechiness by Song Popularity, echo=FALSE}
...
```{r missingness of Tempo by Song Popularity, echo=FALSE}
...
```{r missingness of Audio Valence by Song Popularity, echo=FALSE}
...
```{r boxplot on time signature, echo=FALSE}
...
```{r boxplot on audio mode, echo=FALSE}
...
To evaluate the imputed values a stripplot is visualised. The imputed values, drawn from observed data points closely matching randomly selected donor candidates, are reasonably well distributed across the explanatory variables with missing data points. The blue dots indicate that the data points are from the original data set while the red dots are illustrative of the imputed values. 
...
To select the imputed data set for model building and song popularity predictions, the individual adjusted r-squared values are arranged and output to identify the regression model that would most optimally predict. Thereafter, residual plots are created and Variation Inflation Factor (VIF) values are output to examine the identified model's regression assumptions. 
...
Evaluating VIF values for each independent variable adequetly determines the presence of multicollinearity. A VIF value greater than 5 is indicative of high  multicollinearity whereas values that are greater than 1 and less than or equal to 5 suggest a moderate correlation with other covariates. 
...
A visualisation of relative feature importance in predicting song popularity shows that the five pivotal predictors are Loudness, Energy, Instrumentalness and Audio Valence. 
...
Based on the ANN algoritm, an increase in a track's energ which reflects a songs intensity and activity, is associated with a steady decline in popularity. This suggests that lower energy songs may be more frequently streamed on Spotify. Similarly, as the instrumentalness of a song increases, song popularity initially dips and minimally declines thereafter. There is a clear distinction regarding vocal preference. Listeners likely engage with music that has clear vocal content. As audible vocals taper song popularity songs minimally diverge. In contrast, loudness appears to have a positive relationship with a track's popularity. As loudness increases, song popularity equally increases, with a particularly notable rise beginning around -15 dB. This may indicate the presence of an amplitude “sweet spot” that appeals to keen listeners.
